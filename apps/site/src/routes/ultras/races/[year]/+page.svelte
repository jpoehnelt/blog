<script lang="ts">
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: "Avg Rank",
      cell: ({ row }) => row.original.competitiveness?.averageRank?.toFixed(0) ?? null,
      enableSorting: true,
    },
    {
      id: "eliteCount",
    ...
    <div class="flex flex-col sm:flex-row gap-4">
      ...
      <div class="flex items-center gap-1 bg-stone-100 p-1 rounded-xl ml-2">
        <button
          ...
          class="p-2.5 rounded-lg transition-all {viewMode === 'card' ? 'bg-orange-600 text-white shadow-sm' : 'text-stone-400 hover:text-stone-600'}"
          title="Card view"
        >
        ...
        <button
          ...
          class="p-2.5 rounded-lg transition-all {viewMode === 'table' ? 'bg-orange-600 text-white shadow-sm' : 'text-stone-400 hover:text-stone-600'}"
          title="Table view"
        >
          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {#each table.getHeaderGroups() as headerGroup}
                <tr>
                  {#each headerGroup.headers as header}
                    <th
                      class="text-left px-6 py-4 {header.id === 'location' ? 'hidden sm:table-cell' : ''} {header.id === 'averageRank' || header.id === 'eliteCount' ? 'hidden md:table-cell text-center' : ''}"
                    >
            ...
            <tbody class="divide-y divide-stone-100">
              {#each table.getRowModel().rows as row}
                <tr class="hover:bg-stone-50 transition-colors">
                  ...
                  <td class="px-6 py-4 text-stone-500 text-sm hidden sm:table-cell">{row.original.location}</td>
            ...
            >
              <div class="h-32 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden group-hover:from-slate-800 group-hover:via-slate-700 group-hover:to-slate-800 transition-all duration-300">
                 ...
                 <div class="absolute inset-0 opacity-[0.08]">
                   <svg class="w-full h-full" preserveAspectRatio="xMidYMid slice" viewBox="0 0 200 100" xmlns="http://www.w3.org/2000/svg">
                     <g fill="none" stroke="currentColor" stroke-width="0.8" class="text-white">
                 ...
                 <!-- Subtle glow -->
                 <div class="absolute top-0 right-0 w-24 h-24 bg-orange-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/3"></div>
              ...
              <div class="p-6 pt-4">
                <h2 class="text-xl font-bold text-slate-900 mb-2 group-hover:text-orange-600 transition-colors">{race.title}</h2>
