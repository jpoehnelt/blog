<script lang="ts">
  ...
  let description = "Discover the most competitive ultramarathon races in the United States ranked by elite runner participation. Find races with 10+ elite runners (UltraSignup rank ≥ 90) and compare field strength across events.";
  ...
  let jsonLd = $derived([
    {
      ...
      "@type": "BreadcrumbList",
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Home", item: "https://justin.poehnelt.com/" },
        { "@type": "ListItem", position: 2, name: "Races", item: "https://justin.poehnelt.com/ultras/races" },
        { "@type": "ListItem", position: 3, name: "Competitive", item: "https://justin.poehnelt.com/ultras/races/competitive" },
      ],
    },
    {
      "@context": "https://schema.org",
...
<svelte:head>
  ...
  <meta name="keywords" content="competitive ultramarathon, elite ultra runners, best ultramarathon fields, top ultra races, UltraSignup rankings" />
  <link rel="canonical" href="https://justin.poehnelt.com/ultras/races/competitive" />
  <meta property="og:title" content={title} />
  <meta property="og:description" content={description} />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="https://justin.poehnelt.com/ultras/races/competitive" />
...
<div class="min-h-screen bg-stone-50 pb-20">
    ...
    <div class="relative container mx-auto px-6 py-16 md:py-24">
      <Breadcrumb.Root class="mb-6">
        <Breadcrumb.List>
          ...
          <Breadcrumb.Separator class="text-stone-600" />
          <Breadcrumb.Item>
            <Breadcrumb.Page class="text-white font-medium">Competitive</Breadcrumb.Page>
          </Breadcrumb.Item>
        </Breadcrumb.List>
      </Breadcrumb.Root>
      ...
      <div class="max-w-4xl">
        <div class="flex items-center gap-3 mb-4">
          <span class="px-3 py-1 rounded-full bg-orange-600/20 text-orange-400 text-xs font-semibold tracking-wide border border-orange-600/30">ALL YEARS</span>
        </div>
        ...
        <p class="text-lg md:text-xl text-stone-400 max-w-2xl leading-relaxed">
          Ranking ultramarathons by elite participation—click any column to sort.
        </p>
      </div>
    </div>
  </div>
  ...
  <div class="container mx-auto px-6 -mt-8 relative z-10 space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-stone-200 shadow-sm p-6">
      <h2 class="text-lg font-bold text-slate-900 mb-3">What Makes a Race Competitive?</h2>
                  ...
                  >
                    {#if header.column.getCanSort()}
                      <button
                        ...
                        class="text-xs font-semibold uppercase tracking-wide transition-colors flex items-center gap-1 {header.column.getIsSorted() ? 'text-orange-600' : 'text-stone-500 hover:text-stone-700'}"
                      >
          ...
          <tbody class="divide-y divide-stone-100">
            {#each table.getRowModel().rows as row}
              <tr class="hover:bg-stone-50 transition-colors">
                ...
                <td class="px-6 py-4 text-stone-500 text-sm hidden lg:table-cell">{row.original.location}</td>
                <td class="px-6 py-4 text-stone-500 text-sm">{new Date(row.original.date).toLocaleDateString(undefined, {month:'short', day:'numeric'})}</td>
                <td class="px-6 py-4 text-center">
                  <span class="inline-flex items-center gap-1 bg-orange-50 text-orange-700 px-2 py-1 rounded-full text-xs font-semibold">
    ...
    <div class="mt-12 pt-8 border-t border-stone-200 text-sm text-stone-500 text-center">
      <p>
        ...
        <a href="/ultras/races" class="text-orange-600 hover:text-orange-700 font-medium underline decoration-orange-600/30 hover:decoration-orange-600">Browse by year</a>
      </p>
    </div>
  </div>
</div>
